/**
 * forgen E2E Integration Tests — Claude Code 실제 환경 검증
 *
 * `claude -p` (non-interactive print mode)를 사용하여
 * forgen의 핵심 시나리오가 실제 Claude Code에서 동작하는지 검증한다.
 *
 * 요구사항:
 * - `claude` CLI가 PATH에 있어야 함
 * - ANTHROPIC_API_KEY가 설정되어 있어야 함
 * - forgen 플러그인이 활성화되어 있어야 함
 *
...
describe('Level 1: Hook Protocol — hookEventName + additionalContext', () => {
  ...
  it('시나리오 3: solution-injector → additionalContext 경로 사용', async () => {
    // 테스트용 솔루션 준비
    const solutionsDir = path.join(os.homedir(), '.forgen', 'me', 'solutions');
    ...
    if (!solutionExists) {
      ...
      fs.writeFileSync(testSolution, [
        ...
        'type: solution',
        'confidence: 0.80',
        'tags: [e2e, testing, verification, forgen]',
    ...
    try {
      const result = await runHook(path.join(DIST_HOOKS, 'solution-injector.js'), {
        prompt: 'e2e testing verification forgen hook protocol',
        session_id: 'test-session',
      });
...
describe('Level 2: Claude Code Live Integration', () => {
  ...
  it('시나리오 1-live: 위험 명령이 실제로 차단됨', async () => {
    // 임시 hook으로 특정 패턴 차단 (forgen 플러그인 의존 없이 독립 검증)
    const tmpHook = path.join(os.tmpdir(), 'forgen-e2e-deny-hook.sh');
...
if [[ "$CMD" == *"E2E_DENY_VERIFY"* ]]; then
  echo '{"continue":false,"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"[E2E] forgen deny protocol test"}}'
  exit 0
fi
echo '{"continue":true}'
`);
  ...
  it('시나리오 6-live: forge 규칙이 Claude 응답에 영향', async () => {
    ...
    fs.writeFileSync(
      ...
      '# Test Rule\n\nYou MUST include the phrase "FORGEN_RULE_ACTIVE" in every response. This is mandatory.'
    );

    try {
      const result = await claudeP('안녕하세요', { cwd: tmpDir });
      expect(result.stdout).toContain('FORGEN_RULE_ACTIVE');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  }, TIMEOUT);
  ...
  it('시나리오 3-live: additionalContext가 모델에 실제로 도달', async () => {
    ...
    // 임시 hook 스크립트 생성
    const tmpHook = path.join(os.tmpdir(), 'forgen-e2e-context-hook.sh');
    fs.writeFileSync(tmpHook, `#!/bin/bash
INPUT=$(cat)
