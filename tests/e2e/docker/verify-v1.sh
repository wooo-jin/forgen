...
echo ""
echo "═══════════════════════════════════════════════════════"
echo "  forgen v1 — Full Pipeline E2E Verification"
echo "════════���══════════════════════════════════════════════"
echo ""
...
command -v forgen &>/dev/null && pass "forgen CLI in PATH" || fail "forgen CLI not found"
command -v forgen-mcp &>/dev/null && pass "forgen-mcp in PATH" || fail "forgen-mcp not found"
...
# v1 Profile 생성 확인
V1_PROFILE="$HOME/.forgen/me/forge-profile.json"
...
REC_DIR="$HOME/.forgen/me/recommendations"
...
# v1 세션 상태 파일 생성 확인
SESSIONS_DIR="$HOME/.forgen/state/sessions"
...
if [ -f "$SETTINGS" ]; then
  if node -e "const s=JSON.parse(require('fs').readFileSync('$SETTINGS','utf-8')); process.exit(s.env?.FORGEN_V1 === '1' ? 0 : 1)"; then
    pass "settings.json has FORGEN_V1=1"
  else
    fail "FORGEN_V1 missing from settings.json"
  fi
  if node -e "const s=JSON.parse(require('fs').readFileSync('$SETTINGS','utf-8')); process.exit(s.env?.FORGEN_SESSION_ID ? 0 : 1)"; then
    pass "settings.json has FORGEN_SESSION_ID"
  else
    fail "FORGEN_SESSION_ID missing from settings.json"
  fi
else
...
# Raw log 기록 확인
RAW_LOGS="$HOME/.forgen/state/raw-logs"
...
EVIDENCE_DIR="$HOME/.forgen/me/behavior"
...
PROFILE_OUT=$(forgen inspect profile 2>/dev/null)
...
forgen forge --reset soft 2>/dev/null && pass "forge --reset soft completed" || fail "forge --reset soft failed"
...
HOOKS_DIR=$(find /usr -path "*/forgen/dist/hooks" -type d 2>/dev/null | head -1)
if [ -z "$HOOKS_DIR" ]; then
  HOOKS_DIR=$(find /usr/local -path "*/forgen/dist/hooks" -type d 2>/dev/null | head -1)
fi
