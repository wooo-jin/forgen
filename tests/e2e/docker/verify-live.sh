#!/bin/bash
# forgen — Docker 내부 Claude 실제 동작 검증
# 실제 claude -p 호출로 훅이 작동하는지 확인
# 사용: docker run -e ANTHROPIC_API_KEY=... forgen-e2e-live
...
echo ""
echo "═══════════════════════════════════════════════════"
echo "  forgen — Live Claude Integration (Docker)"
echo "═══════════════════════════════════════════════════"
echo ""
...
RESULT=$(claude -p "Run 'echo FORGEN_LIVE_TEST' using the Bash tool and show the output" --allowedTools Bash 2>&1 || true)
if echo "$RESULT" | grep -q "FORGEN_LIVE_TEST"; then
...
# .claude/rules/ 에 forgen 규칙이 주입되어 있는지
if [ -d "/workspace/test-project/.claude/rules" ]; then
  ...
  # forgen를 한번도 harness로 안 돌렸으면 rules 없을 수 있음
...
HOOKS_BASE=$(find /root/.claude/plugins/cache/forgen-local -name "intent-classifier.js" -path "*/hooks/*" 2>/dev/null | head -1)
...
FILTER_JS=$(find /root/.claude/plugins/cache/forgen-local -name "prompt-injection-filter.js" -path "*/hooks/*" 2>/dev/null | head -1)
if [ -n "$FILTER_JS" ]; then
...
TUNER_JS=$(find /root/.claude/plugins/cache/forgen-local -name "auto-tuner.js" -path "*/forge/*" 2>/dev/null | head -1)
if [ -n "$TUNER_JS" ]; then
  TUNER_RESULT=$(node -e "
