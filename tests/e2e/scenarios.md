...
3. `~/.claude.json`에 `forgen-compound` MCP가 있다
...
**검증 체인:**
1. `forgen forge` → 인터뷰 답변
2. `~/.forgen/me/forge-profile.json` 생성 (차원값 0-1)
...
2. **prompt-learner가 이 패턴을 감지하는가** → `~/.forgen/me/behavior/` 파일 생성 확인
...
4. `~/.forgen/me/solutions/`에 **새 파일 생성** (이전 대비 +N)
...
3. **이 패턴이 Claude의 시스템 프롬프트에 도달하는가**
   - `~/.forgen/me/USER.md` → 어떤 경로로 Claude에 전달?
...
**검증 체인:**
1. verified 솔루션이 있음
2. `forgen skill promote <name>` 실행
3. `.forgen/me/skills/<name>.md` 생성
