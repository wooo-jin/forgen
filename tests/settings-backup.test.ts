...
const TEST_DIR = path.join(os.tmpdir(), 'forgen-test-settings-backup');
const TEST_SETTINGS_PATH = path.join(TEST_DIR, 'settings.json');
const TEST_BACKUP_PATH = path.join(TEST_DIR, 'settings.json.forgen-backup');
...
function injectSettingsTo(dir: string, env: Record<string, string>): void {
  ...
  const backupPath = path.join(dir, 'settings.json.forgen-backup');
...
describe('injectSettings — 백업 생성', () => {
  it('settings.json이 존재하면 .forgen-backup 파일이 생성된다', () => {
...
describe('rollbackSettings — 복원 동작', () => {
  it('.forgen-backup이 있으면 settings.json을 복원하고 true를 반환한다', () => {
  ...
  it('복원 후 .forgen-backup 파일은 삭제된다', () => {
  ...
  it('.forgen-backup이 없으면 false를 반환한다', () => {
    const result = rollbackSettingsIn(TEST_DIR);
