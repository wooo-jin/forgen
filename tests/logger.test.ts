...
describe('Logger', () => {
  ...
  beforeEach(() => {
    ...
    // 환경변수 초기화
    delete process.env.FORGEN_LOG_LEVEL;
    delete process.env.FORGEN_DEBUG;
  ...
  describe('LogLevel 제어 (FORGEN_LOG_LEVEL)', () => {
    it('기본 레벨(info)에서는 info/warn/error만 출력된다', () => {
      // FORGEN_LOG_LEVEL 미설정 → info가 기본
      process.env.FORGEN_DEBUG = '*';
      const logger = createLogger('test');
    ...
    it('FORGEN_LOG_LEVEL=debug 설정 시 debug 메시지가 출력된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'debug';
      process.env.FORGEN_DEBUG = '*';
      const logger = createLogger('test');

      logger.debug('디버그 메시지');
      expect(stderrSpy).toHaveBeenCalledWith(
        expect.stringContaining('[forgen:test] [debug] 디버그 메시지')
      );
    });

    it('FORGEN_LOG_LEVEL=warn 설정 시 warn/error만 출력된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'warn';
      const logger = createLogger('test');
    ...
    it('FORGEN_LOG_LEVEL=error 설정 시 error만 출력된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'error';
      const logger = createLogger('test');
  ...
  describe('namespace 필터링 (FORGEN_DEBUG)', () => {
    it('FORGEN_DEBUG 미설정 시 debug 메시지는 출력되지 않는다', () => {
      process.env.FORGEN_LOG_LEVEL = 'debug';
    ...
    it('FORGEN_DEBUG=* 설정 시 모든 namespace debug가 출력된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'debug';
      process.env.FORGEN_DEBUG = '*';
    ...
    it('FORGEN_DEBUG=provider 설정 시 provider만 출력된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'debug';
      process.env.FORGEN_DEBUG = 'provider';
      createLogger('provider').debug('프로바이더');
      createLogger('hook').debug('훅');
      expect(stderrSpy).toHaveBeenCalledTimes(1);
      expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('[forgen:provider]'));
    });

    it('FORGEN_DEBUG=provider,hook 쉼표 구분 시 해당 namespace만 출력된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'debug';
      process.env.FORGEN_DEBUG = 'provider,hook';
      createLogger('provider').debug('프로바이더');
      createLogger('hook').debug('훅');
  ...
  describe('출력 형식', () => {
    it('[forgen:namespace] [level] message 형식으로 출력된다', () => {
      const logger = createLogger('provider');
      logger.info('테스트 메시지');
      expect(stderrSpy).toHaveBeenCalledWith('[forgen:provider] [info] 테스트 메시지');
    });
    ...
    it('error 인자가 있으면 ": message" 형식으로 이어진다', () => {
      ...
      expect(stderrSpy).toHaveBeenCalledWith(
        expect.stringContaining('[forgen:test] [error] 처리 실패: 연결 거부')
      );
    });
  ...
  describe('레거시 호환 (debugLog)', () => {
    ...
    it('FORGEN_DEBUG=* 설정으로도 debugLog가 활성화된다', () => {
      process.env.FORGEN_LOG_LEVEL = 'debug';
      process.env.FORGEN_DEBUG = '*';
      debugLog('legacy', '레거시');
    ...
    it('FORGEN_LOG_LEVEL=warn일 때 debugLog는 출력되지 않는다', () => {
      process.env.FORGEN_LOG_LEVEL = 'warn';
      process.env.COMPOUND_DEBUG = '1';
