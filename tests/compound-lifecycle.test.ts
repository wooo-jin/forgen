...
const { TEST_HOME } = vi.hoisted(() => ({
  TEST_HOME: '/tmp/forgen-test-lifecycle',
}));
...
vi.mock('../src/core/paths.js', () => {
  ...
  const BASE = '/tmp/forgen-test-lifecycle';
...
const SOLUTIONS_DIR = path.join(TEST_HOME, '.forgen', 'me', 'solutions');
...
describe('checkIdentifierStaleness()', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync('/tmp/forgen-stale-');
  ...
  it('길이 4 미만인 식별자는 검사를 건너뛴다 — 빈 코드베이스에서도 true(stale)를 반환한다', () => {
    const emptyDir = fs.mkdtempSync('/tmp/forgen-empty-');
    try {
