...
const { TEST_HOME } = vi.hoisted(() => ({
  TEST_HOME: '/tmp/forgen-test-solution-matcher-full',
}));
...
const ME_SOLUTIONS = path.join(TEST_HOME, '.forgen', 'me', 'solutions');
...
describe('matchSolutions', () => {
  ...
  it('팀 솔루션도 매칭한다', () => {
    const teamPackDir = path.join(TEST_HOME, '.forgen', 'packs', 'my-team', 'solutions');
