...
describe('ForgenError', () => {
  it('기본 필드가 설정된다', () => {
    const err = new ForgenError('something went wrong');
    expect(err.message).toBe('something went wrong');
    expect(err.name).toBe('ForgenError');
    expect(err.code).toBe('FORGEN_ERROR');
    expect(err.context).toEqual({});
  });

  it('code와 context를 옵션으로 받는다', () => {
    const err = new ForgenError('fail', { code: 'CUSTOM', context: { key: 'val' } });
    expect(err.code).toBe('CUSTOM');
    expect(err.context).toEqual({ key: 'val' });
  });

  it('Error를 상속한다', () => {
    const err = new ForgenError('x');
    expect(err).toBeInstanceOf(Error);
    expect(err).toBeInstanceOf(ForgenError);
  });

  it('toJSON()이 직렬화 가능한 객체를 반환한다', () => {
    const err = new ForgenError('fail', { code: 'C1', context: { a: 1 } });
...
describe('ProviderError', () => {
  ...
  it('instanceof 체인: ProviderError → ForgenError → Error', () => {
...
describe('HookError', () => {
  ...
  it('instanceof 체인: HookError → ForgenError → Error', () => {
...
describe('ConfigError', () => {
  ...
  it('configPath와 field를 옵션으로 받는다', () => {
    const err = new ConfigError('missing field', { configPath: '/etc/forgen.json', field: 'apiKey' });
    expect(err.configPath).toBe('/etc/forgen.json');
    expect(err.field).toBe('apiKey');
  });

  it('instanceof 체인: ConfigError → ForgenError → Error', () => {
...
describe('PackError', () => {
  ...
  it('instanceof 체인: PackError → ForgenError → Error', () => {
...
describe('ForgeError', () => {
  ...
  it('instanceof 체인: ForgeError → ForgenError → Error', () => {
...
describe('NonRetryableError', () => {
  ...
  it('instanceof 체인: NonRetryableError → ForgenError → Error', () => {
